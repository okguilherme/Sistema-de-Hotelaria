...
public class TesteFileStreams {
    ...
    public static void main(String[] args) {
        ...
        try (FileOutputStream fos = new FileOutputStream(NOME_ARQUIVO);
                ReservaArrayOutputStream resOut = new ReservaArrayOutputStream(fos, reservasParaEscrever,
                        reservasParaEscrever.length)) {

            // Vamos usar um loop longo para garantir que todas as reservas são processadas
            // e escritas.
            for (int i = 0; i < 2000; i++) { // Número grande para cobrir todas as reservas
                try {
                    resOut.write(0); // O valor do byte não importa
                } catch (IOException e) {
                    // Quando não houver mais bytes internos para escrever, pode lançar exceção ou
                    // parar.
                    break;
                }
            }
